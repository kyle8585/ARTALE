            ...
            for p in filtered:
                ...
                with st.expander(label):
                    ...
                    with col_info:
                        ...
                        with b1:  # 加入功能
                            if not is_owner:
                                with st.popover("➕ 加入", use_container_width=True):
                                    m_job = st.selectbox("職業", all_jobs, key=f"j_{cat_name}_{p['id']}")
                                    m_lvl = st.number_input("等級", 1, 200, 100, key=f"l_{cat_name}_{p['id']}")
                                    if st.button("確認加入", key=f"btn_{cat_name}_{p['id']}", use_container_width=True):
                                ...
                                st.button("隊長本人", disabled=True, use_container_width=True,
                                          key=f"owner_btn_{cat_name}_{p['id']}")

                        with b2:  # 修改功能 (限隊長)
                            if is_owner:
                                with st.popover("✏️ 修改", use_container_width=True):
                                    new_t = st.text_input("修改標題", value=p['title'], key=f"et_{cat_name}_{p['id']}")
                                    new_n = st.text_input("修改備註", value=p['note'], key=f"en_{cat_name}_{p['id']}")
                                    if st.button("更新", key=f"up_{cat_name}_{p['id']}", use_container_width=True):
                        ...
                        with b3:  # 撤團功能 (限隊長)
                            if is_owner:
                                if st.button("🗑️ 撤團", key=f"del_{cat_name}_{p['id']}", type="primary",
                    ...
                    with col_chat:
                        ...
                        with st.form(key=f"chat_form_{cat_name}_{p['id']}", clear_on_submit=True):
                            ...
                            u_msg = c1.text_input("訊息", placeholder="說點什麼...", key=f"msg_in_{cat_name}_{p['id']}")
